...
func TestMutateRaceTable(t *testing.T) {
	target := initTargetTest(t, "test", "64")
	tests := [][1]string{
		{`
mutate0()
...
`},
	...
	for ti, test := range tests {
		...
		t.Run(fmt.Sprint(ti), func(t *testing.T) {
			...
			for ri0 := 0; ri0 <= len(p.Calls)-2; ri0++ {
				for ri1 := ri0 + 1; ri1 <= len(p.Calls)-1; ri1++ {
					indices := [2]int{ri0, ri1}
					for i := 0; i < 10000; i++ {
						p1 := p.Clone()
						newIndices := p1.MutateRace(rs, 30, ct, nil, indices)

						for k := 0; k < 2; k++ {
							if p.Calls[indices[k]].Meta.Name != p1.Calls[newIndices[k]].Meta.Name {
								data1 := p1.Serialize()
								errstr := fmt.Sprintf("failed to adjust index (iter %v)\n", i)
								errstr += fmt.Sprintf("[*] orig (indices %v):\n%s\n", indices, test[0])
								errstr += fmt.Sprintf("[*] mutated (indices %v):\n%s\n", newIndices, string(data1))
								t.Fatalf(errstr)
							}
						}
					}
