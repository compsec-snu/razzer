...
#define swap(a, b)                  \
	do {                        \
		...
		swap(race_info[0].race_index, race_info[1].race_index);
		swap(race_info[0].bp, race_info[1].bp);
	}
	for (int i = 0; i < 2; i++) {
		race_done[i] = 0;
	}
...
static inline int finish_race()
{
	...
	if (race_result[0] != race_result[1]) {
		...
		mylogf("Executor race result (%llx %llx) : %d\n", race_info[0].bp, race_info[1].bp, ret);
	}
	return ret;
}

// execute_one executes program stored in input_data with raceInfo stored in race_info.
// return race result
int execute_one()
{
...
retry:
	...
	for (;;) {
		...
		if (colliding /* && (call_index % 2) == 0 */) {
			// In syz-fuzzer, we don't have to wait all syscalls when colliding
...
inline void set_cpu_affinity(thread_t* th)
{
...
#define gettid() syscall(SYS_gettid)
			mylogf("[%d] TID: %d", th->id, gettid());
			mylogf("[%d] hypercall CMD_REFRESH", th->id);
...
void execute_call(thread_t* th)
{
	...
	if (th->colliding)
		mylogf("[%d] call #%d: %s", th->id, th->call_index, call->name);
	...
	if (th->executing_racy_syscall) {
		...
		mylogf("[%d] hypercall CMD_START", th->id);
		mylogf("[%d] \tInstall bp: 0x%lx", th->id, bp);
		mylogf("[%d] \tsched: %d", th->id, sched);
	...
	if (th->executing_racy_syscall) {
		...
		mylogf("[%d] hypercall CMD_END", th->id);
		uint64 is_race = hypercall(th->id, CMD_END, 0, 0);
		mylogf("[%d] \tis_race: %d", th->id, is_race);
		if (is_race == (uint64)-1) {
			doexit(kHypercallFail);
		} else {
