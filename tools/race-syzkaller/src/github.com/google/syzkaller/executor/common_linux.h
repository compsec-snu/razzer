...
#include <sys/mount.h>
#endif
#if defined(SYZ_EXECUTOR) || defined(SYZ_TUN_ENABLE) || defined(SYZ_RACE_REPRO)
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
#include <linux/if.h>
...
#if defined(SYZ_RACE_REPRO)

static bool should_hypercall = false;
static uint64 is_race[2];
static void* execute_thr(void *arg);

pthread_barrier_t race_barrier;

static inline uint64 repro_hypercall(long id, long cmd, unsigned long bp, int sched)
{
  uint64 ret = syscall(SYS_hypercall, id, cmd, bp, sched);
  return ret;
}

#define RACE_INIT 0xffff
#define RACE_ERR (uint64)-1
#define RACE_FAIL 0
#define RACE_OK 1
...
void race_hcall_setup(long arg, long sched_order) {
  if (should_hypercall) {
    uint64 res_hcall = repro_hypercall((long)arg, CMD_START, bps[(int)arg], sched_order);
    if (res_hcall == (uint64)-1) {
...
void race_hcall_mark_and_check(long arg)
{
  if (should_hypercall) {
    ...
    uint64 res_hcall = repro_hypercall((long)arg, CMD_END, 0, 0);
...
static void loop()
{
	...
	for (iter = 0;; iter++) {
		...
		if (pid == 0) {
			...
			int race_result = execute_one();
			send_race_result(race_result);
...
#if defined(SYZ_THREADED) && !defined(SYZ_RACE_REPRO)
struct thread_t {
