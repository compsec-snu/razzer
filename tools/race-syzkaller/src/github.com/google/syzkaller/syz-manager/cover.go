...
var (
	...
	allAddrToFunc   = make(map[uint64]string)
...
func initAllCover(os, arch, vmlinux string) {
	...
	// RYONG: we need the allCoverPCs before all what I implemented, so I make it synchronously
	syncpcs, pcs, err := coveredPCs(arch, vmlinux)
	if err == nil {
		sort.Sort(uint64Array(pcs))
		sort.Sort(uint64Array(syncpcs))
		allCoverPCs = pcs
		allSyncPCs = syncpcs
	} else {
...
func coveredPCs(arch, bin string) ([]uint64, []uint64, error) {
	...
	for s.Scan() {
		lock := false
		ln := s.Bytes()
		if pos := bytes.IndexByte(ln, ':'); pos == len(ln)-1 && pos != -1 {
			// Function definition
			curFun = string(r.Find(ln))
			isFirst = true
			continue
		}
		...
		if lock {
			lockpcs = append(lockpcs, pc)
		} else {
			if isFirst {
				isFirst = false
				/* kcov records the next PC of the callq __sanitizer_cov_trace_pc */
				allAddrToFunc[pc+5] = curFun[1 : len(curFun)-1]
			}
			pcs = append(pcs, pc)
		}
	}
...
func findItem(allCoverPCs []uint64, item uint64) (uint64, error) {
	if allCoverPCs[0] > item {
		return 0, fmt.Errorf("Error")
	}
	left := 0
	right := len(allCoverPCs) - 1
	for left <= right {
		if allCoverPCs[right] <= item {
			return allCoverPCs[right], nil
		}
		mid := (left + right) / 2
		if allCoverPCs[mid] <= item {
			if left == mid {
				return allCoverPCs[mid], nil
			}
			left = mid
		} else {
			right = mid - 1
...
func getCoverPerMapping(sourceLoc string, mappingTag string, items []uint64) (map[LocTy][]uint32, map[string][]ItemTy, error) {
	// TODO: find a pc less then each item, and make mapping bb -> key
	// Too messy...

	locCovToAddr := make(map[string][]ItemTy)
	locToBB := make(map[LocTy][]uint32)

	for i := 0; i < len(mappingTag); i++ {
		// Split tag into one character. e.g., RW -> R, W
		tag := string(mappingTag[i])
		key := LocTy{SourceLoc: sourceLoc, Tag: tag}

		tempHash := make(map[uint32]struct{})
		for _, item := range items {

			// Calculate BB
			bb_, err := findItem(allCoverPCs, item)
			if err != nil {
				return nil, nil, err
			}
			bb := uint32(bb_) + 5 /* kcov records the next PC of the callq __sanitizer_cov_trace_pc */

			hshString := fmt.Sprintf("%s%x", sourceLoc, bb)
			hsh := hash.String([]byte(hshString))
			locCovToAddr[hsh] = append(locCovToAddr[hsh], ItemTy{Item: uint32(item), Tag: tag})

			if _, ok := tempHash[bb]; ok {
				continue
			}
			tempHash[bb] = struct{}{}
			locToBB[key] = append(locToBB[key], uint32(bb))
		}
	}
	return locToBB, locCovToAddr, nil
}
