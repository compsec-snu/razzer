...
func DefaultConfig(sched bool) (*Config, *ExecOpts, error) {
	Assert(sched == *flagCollide, "Config and *flagCollide have a different value")
	c := &Config{
...
func (env *Env) ExecRace(opts *ExecOpts, p *prog.Prog, raceInfo RaceInfo) (output []byte, info []CallInfo, failed, hanged, isRace bool, err0 error) {
	...
	var restart bool
	output, failed, hanged, restart, isRace, err0 = env.cmd.execRace(opts, progData, raceInfo)
	if err0 != nil || restart {
		env.cmd.close()
		env.cmd = nil
		return
...
type executeReq struct {
	...
	// RaceInfo
	// serializeUint64(inCmd[24:], uint64(opts.BP0))
	// serializeUint64(inCmd[32:], uint64(opts.BP1))
	// serializeUint64(inCmd[40:], uint64(opts.RaceIndex0))
	// serializeUint64(inCmd[48:], uint64(opts.RaceIndex1))
	// serializeUint64(inCmd[64:], uint64(opts.Sched))
	bp0        uint64
	bp1        uint64
	raceIndex0 uint64
	raceIndex1 uint64
	...
	if config.Flags&FlagDebug != 0 {
		...
		go func(c *command) {
			...
			for {
				...
				lines := strings.Split(string(output[size:size+uint64(n)]), "\n")
				for _, line := range lines {
					if len(strings.TrimSpace(line)) == 0 {
						continue
					}
					Logf(0, "[EXECUTOR] %s", line)
				}
				if n > 0 {
					size += uint64(n)
					if size >= bufSize*3/4 {
...
func (c *command) execRace(opts *ExecOpts, progData []byte, raceInfo RaceInfo) (output []byte, failed, hanged,
	restart, isRace bool, err0 error) {
	req := &executeReq{
		magic:      inMagic,
		...
		bp0:        uint64(raceInfo.Addr[0]),
		bp1:        uint64(raceInfo.Addr[1]),
		raceIndex0: uint64(raceInfo.Idx[0]),
		raceIndex1: uint64(raceInfo.Idx[1]),
		sched:      uint64(raceInfo.Sched),
	}
	reqData := (*[unsafe.Sizeof(*req)]byte)(unsafe.Pointer(req))[:]
	if _, err := c.outwp.Write(reqData); err != nil {
		output = <-c.readDone
