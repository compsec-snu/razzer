...
func Write(p *prog.Prog, opts Options) ([]byte, error) {
	...
	if !opts.Repeat {
		...
		if opts.Race {
			bps := [2]uint32{0, 0}
			if opts.RaceAddr0 != 0 {
				bps[0] = opts.RaceAddr0
			}
			if opts.RaceAddr1 != 0 {
				bps[1] = opts.RaceAddr1
			}
			ctx.printf("\tbps[0] = 0x%x;\n", bps[0])
			ctx.printf("\tbps[1] = 0x%x;\n", bps[1])

			ctx.printf("\tif (argc > 1) should_hypercall = true;\n")
			ctx.printf("\tif (should_hypercall) {\n")
			ctx.printf("\t\tdebug(\"[*] hypercall enabled: pid (%%d)\\n\", getpid());\n")
			ctx.printf("\t} else {\n")
			ctx.printf("\t\tdebug(\"[*] hypercall disabled: pid (%%d)\\n\", getpid());\n")
			ctx.printf("\t}\n")
		}
...
func (ctx *context) generateExecuteCallNormal(calls []string, hasVars bool) {
...
func (ctx *context) generateExecuteCallRace(calls []string) {
	if ctx.opts.RaceIndex0 >= ctx.opts.RaceIndex1 {
		panic("Incorrect RaceIndex0/RaceIndex1")
	}
	if ctx.opts.RaceIndex1 >= len(calls) {
		panic("RaceIndex1 is too big")
	}

	ctx.printf("\n")
	ctx.printf("void race_hcall_prepare(long);\n")
	ctx.printf("void race_hcall_setup(long, long);\n")
	ctx.printf("void race_hcall_mark_and_check(long);\n")
	ctx.printf("\n")

	ctx.printf("void* execute_thr(void *arg)\n{\n")
	ctx.printf("\tstatic int count = 0;\n")
	ctx.printf("\tuint64_t tid = (uint64_t)arg;\n")
	ctx.printf("\tint res = 0;\n")
	ctx.printf("\tuint64_t turn = 0;\n")
	ctx.printf("\n")
	ctx.printf("\twhile (1) {\n")

	ctx.printf("\t\tif (count++ %% 100 == 0) {\n")
	ctx.printf("\t\t\tdebug(\".\");\n")
	ctx.printf("\t\t}\n\n")

	ctx.printf("\t\tturn = (count + tid) %% 2;\n")

	ctx.printf("\t\trace_hcall_prepare(tid);\n")

	// emit pre-race calls
	ctx.printf("\t\tif (tid == 0) {\n")
	for _, c := range calls[:ctx.opts.RaceIndex0] {
		ctx.printf("%s", strings.Replace(c, "\t", "\t\t\t", -1))
	}
	// ctx.printf("\t\t\tpthread_barrier_wait(&race_barrier);\n\n")
	ctx.printf("\t\t}\n") // end of if (tid == 0)

	ctx.printf("\t\tif (tid == 1) {\n")
	// ctx.printf("\t\t\tpthread_barrier_wait(&race_barrier);\n\n")
	for _, c := range calls[ctx.opts.RaceIndex0+1 : ctx.opts.RaceIndex1] {
		ctx.printf("%s", strings.Replace(c, "\t", "\t\t\t", -1))
	}
	ctx.printf("\t\t}\n\n") // end of if (tid == 1)

	// TODO: randomize the second parameter (sched order)
	ctx.printf("\t\trace_hcall_setup(tid, 0);\n")

	// barrier
	ctx.printf("\t\tpthread_barrier_wait(&race_barrier);\n\n")

	// emit racy calls
	ctx.printf("\t\tswitch (tid) {\n")
	ctx.printf("\t\tcase 0:\n")
	ctx.printf("\t\tcase 2:\n")
	ctx.printf("%s", strings.Replace(calls[ctx.opts.RaceIndex0], "\t", "\t\t\t", -1))
	ctx.printf("\t\t\tbreak;\n")
	ctx.printf("\t\tcase 1:\n")
	ctx.printf("\t\tcase 3:\n")
	ctx.printf("%s", strings.Replace(calls[ctx.opts.RaceIndex1], "\t", "\t\t\t", -1))
	ctx.printf("\t\t\tbreak;\n")
	ctx.printf("\t\t}\n\n") // end of switch

	ctx.printf("\t\trace_hcall_mark_and_check(tid);\n")

	// emit post-race calls
	ctx.printf("\t\tif (turn == 1) {\n")
	for _, c := range calls[ctx.opts.RaceIndex1+1:] {
		ctx.printf("%s", strings.Replace(c, "\t", "\t\t\t", -1))
	}
	ctx.printf("\t\t}\n\n") // end of if (tid == 0)
	...
	// barrier
	ctx.printf("\t\tif (rand()%%1000 <= 1) {\n")
	ctx.printf("\t\t\tdebug(\"Exiting\\n\", count);\n")
	ctx.printf("\t\t\texit(0);\n")
	ctx.printf("\t\t}\n")

	// we will only try once if using hypercall.
	ctx.printf("\t\tif (should_hypercall) break;\n")
...
func (ctx *context) generateTestFunc(calls []string, hasVars bool, name string) {
	...
	if !opts.Threaded && !opts.Collide {
		...
		if !opts.Race {
			ctx.generateExecuteCallNormal(calls, hasVars)
		} else {
			ctx.generateExecuteCallRace(calls)
		}
