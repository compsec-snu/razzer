...
func TestParseOptionsCanned(t *testing.T) {
	...
	canned := map[string]Options{
		"{Threaded:true Collide:true Repeat:true Procs:1 Sandbox:none Fault:false FaultCall:-1 FaultNth:0 EnableTun:true UseTmpDir:true HandleSegv:true WaitRepeat:true Debug:false Repro:false RaceIndex0:0 RaceIndex1:1 RaceAddr0:0 RaceAddr1:0 Race:true}": Options{
			Threaded:      true,
			Collide:       true,
			Repeat:        true,
			Procs:         1,
		...
		"{Threaded:true Collide:true Repeat:true Procs:1 Sandbox: Fault:false FaultCall:-1 FaultNth:0 EnableTun:true UseTmpDir:true HandleSegv:true WaitRepeat:true Debug:false Repro:false RaceIndex0:1 RaceIndex1:1 RaceAddr0:1234 RaceAddr1:5678 Race:true}": Options{
			Threaded:      true,
			Collide:       true,
			Repeat:        true,
			Procs:         1,
		...
		"{Threaded:false Collide:true Repeat:true Procs:1 Sandbox:namespace Fault:false FaultCall:-1 FaultNth:0 EnableTun:true UseTmpDir:true EnableCgroups:true HandleSegv:true WaitRepeat:true Debug:false Repro:false RaceIndex0:10 RaceIndex1:20 RaceAddr0:1234 RaceAddr1:5678 Race:true}": Options{
			Threaded:      false,
			Collide:       true,
			Repeat:        true,
			Procs:         1,
...
func enumerateField(opt Options, field int) []Options {
	...
	if fldName == "Sandbox" {
		...
		opts = append(opts, opt)
	} else if fldName == "RaceIndex0" {
		opts = append(opts, opt)
	} else if fldName == "RaceIndex1" {
		opts = append(opts, opt)
	} else if fldName == "RaceAddr0" {
		opts = append(opts, opt)
	} else if fldName == "RaceAddr1" {
		opts = append(opts, opt)
