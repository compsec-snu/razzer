	...
	if report {
		...
		if n != 0 {
			...
			for kmemleakBuf = kmemleakBuf[:n]; len(kmemleakBuf) != 0; {
				...
				log.Logf(0, "[SCHED] BUG: memory leak\n%s\n", report)
				nleaks++
			}
			if nleaks != 0 {
				os.Exit(1)
...
func checkCompsSupported() (kcov, comps bool) {
	...
	if errno != 0 {
		log.Logf(1, "[SCHED] KCOV_CHECK: KCOV_INIT_TRACE = %v", errno)
		return
	}
	mem, err := syscall.Mmap(fd, 0, int(coverSize*unsafe.Sizeof(uintptr(0))),
		syscall.PROT_READ|syscall.PROT_WRITE, syscall.MAP_SHARED)
	if err != nil {
		log.Logf(1, "[SCHED] KCOV_CHECK: mmap = %v", err)
		return
	}
	...
	if errno != 0 {
		log.Logf(1, "[SCHED] KCOV_CHECK: KCOV_ENABLE = %v", errno)
		return
	}
