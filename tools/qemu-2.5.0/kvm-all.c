...
int kvm_init_hypercall(CPUState *cpu) {
    return kvm_insert_breakpoint_per_cpu(cpu, _HYPERCALL_ADDR);
}

#define gettid() \
	syscall(SYS_gettid)

extern QemuMutex qemu_race_mutex;

static void update_registers(CPUState *cpu) {
    if (cpu->update_regs) {
        kvm_vcpu_ioctl(cpu, KVM_SET_REGS, cpu->regs);
        free(cpu->regs);
        cpu->regs = NULL;
    }
}

static void read_registers(CPUState *cpu) {
    struct kvm_regs *regs = (struct kvm_regs*)malloc(sizeof(struct kvm_regs));
    int err = kvm_vcpu_ioctl(CPU(cpu), KVM_GET_REGS, regs);
    if (err < 0)
        panic("ioctl error\n");

    cpu->regs = regs;
    cpu->update_regs = false;
}
...
int kvm_cpu_exec(CPUState *cpu)
{
    ...
    do {
        ...
        if (ret == EXCP_DEBUG) {
            hcall_type_t type = check_hypercall(cpu);
            switch(type) {
                case HYPERCALL_COMMAND:
                    handle_hypercall_command(cpu);
                    ret = 0;
                    break;
                case HYPERCALL_BREAKPOINT:
                    handle_hypercall_breakpoint(cpu);
                    ret = 0;
                    break;
                case HYPERCALL_UNEXPECTED:
                    panic("Unexpected hypercall");
                    break;
                default:
                    break;
            }
...
int kvm_update_guest_debug_per_cpu(CPUState *cpu, unsigned long reinject_trap)
{
    ...
    kvm_invoke_set_guest_debug(&data);
    return data.err;
}
...
int kvm_insert_breakpoint_per_cpu(CPUState *cpu, target_ulong addr) {
    struct kvm_set_guest_debug_data data;
    int err = kvm_arch_insert_hw_breakpoint_per_cpu(cpu, addr);
    if (err) {
        return err;
    }
...
int kvm_remove_breakpoint_per_cpu(CPUState *cpu, target_ulong addr) {
    struct kvm_set_guest_debug_data data;
    int err = kvm_arch_remove_hw_breakpoint_per_cpu(cpu, addr);
    if (err) {
        return err;
    }
...
void kvm_remove_all_breakpoints_per_cpu(CPUState *cpu)
{
    struct kvm_set_guest_debug_data data;
	cpu->nb_hw_bps = 0;
    kvm_arch_update_guest_debug(cpu, &(data.dbg));
