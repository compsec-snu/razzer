...
static int find_hw_breakpoint_per_cpu(CPUState *cpu, target_ulong addr)
{
    int n;
    const target_ulong len = 1;
    const int type = GDB_BREAKPOINT_HW;

    for (n = 0; n < cpu->nb_hw_bps; n++) {
        if (cpu->hw_bps[n].addr == addr && cpu->hw_bps[n].type == type &&
            (cpu->hw_bps[n].len == len || len == -1)) {
            return n;
        }
    }
    return -1;
...
int kvm_arch_insert_hw_breakpoint_per_cpu(CPUState *cpu, target_ulong addr)
{
    const int type = GDB_BREAKPOINT_HW;
    const target_ulong len = 1;
    ...
    if (find_hw_breakpoint_per_cpu(cpu, addr) >= 0) {
        return -EEXIST;
    }
    cpu->hw_bps[cpu->nb_hw_bps].addr = addr;
    cpu->hw_bps[cpu->nb_hw_bps].len = len;
    cpu->hw_bps[cpu->nb_hw_bps].type = type;
    cpu->nb_hw_bps++;
    return 0;
}
                                  ...
                                  target_ulong len, int type)
{
    ...
    int n;
    for(n=0;n<nb_hw_breakpoint;n++) {
        fprintf(stderr, "%lx %u\n", hw_breakpoint[n].addr, hw_breakpoint[n].type);
    }

    return 0;
}

int kvm_arch_remove_hw_breakpoint_per_cpu(CPUState *cpu, target_ulong addr)
{
    int n;

    n = find_hw_breakpoint_per_cpu(cpu, addr);
    if (n < 0) {
        return -ENOENT;
    }
    cpu->nb_hw_bps--;
    cpu->hw_bps[n] = cpu->hw_bps[cpu->nb_hw_bps];
    return 0;
}
...
void kvm_arch_update_guest_debug(CPUState *cpu, struct kvm_guest_debug *dbg)
{
    ...
    if (cpu->nb_hw_bps > 0) {
        ...
        for (n = 0; n < cpu->nb_hw_bps; n++) {
            dbg->arch.debugreg[n] = cpu->hw_bps[n].addr;
            dbg->arch.debugreg[7] |= (2 << (n * 2)) |
                (type_code[cpu->hw_bps[n].type] << (16 + n*4)) |
                ((uint32_t)len_code[cpu->hw_bps[n].len] << (18 + n*4));
        }
    }
}
