...
../configure --target-list=x86_64-softmmu --cc=$GCC7_HOME/install/bin/gcc --cxx=$GCC7_HOME/install/bin/g++

CC=$GCC7_HOME/install/bin/gcc CFLAGS="-D_HYPERCALL_ADDR=$HYPEADDR" make -j`nproc`
